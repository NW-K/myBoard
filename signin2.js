		...
		if(db){
			authUser(db, paramId, paramPW, function(err, result){
				...
				if(result){
					...
					res.redirect('/');
					// res.writeHead('200', {'Content-Type':'text/html; charset=utf8'});
					// res.write(req.session.nickname+'님 안녕하세요');
					// res.write('<a href="/">');
...
var signIn = function(db,id, password, name, nickname, phone, date, callback){
	...
	var register = {"id":id, "password":password, "name":name, "nickname":nickname, "phone":phone, "date":date};
				...
				if(result){
					...
					showPage(queryId, function(err, result){
						...
						if(result){
							...
							res.render('board', {Number:'Number(i+1)', curWriter: 'curWriter', curDate:'curDate', curTitle:'curTitle', Number2:'Number(queryId-1)', j:'j', Number3:'Number(queryId+1)'})
							// res.writeHead('200', {'Content-Type':'text/html; charset=utf8'});
